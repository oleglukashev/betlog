...
class Coefficients(Base):
  ...
  def __repr__(self):
    return "<Coefficient('%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d')>" % ( self.event_id, self.bookmaker_id, self.first, self.draw, self.second, self.first_or_draw, self.first_or_second, self.draw_or_second, self.first_fora, self.second_fora, self.total_less, self.total_more )
